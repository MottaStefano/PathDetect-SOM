...
for(i in 1:ncol(CENTROID)){
    ...
    write(paste(CLUS_NAME[i], ": Neuron ",  ReprNeuron[i], sep=''), file=paste(FOLDER, "/Cluster_Representatives.dat", sep=''), append=TRUE)
}
cat("Done\n")
...
write_graph(D, file=paste(FOLDER, "/", OUT, "_Transition_Network-noDiagonal.xml", sep=''), format="graphml")
